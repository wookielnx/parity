...
//			}
	...
	fn on_close_block(&self, block: &mut ExecutedBlock) {
		...
		// Bestow block reward
		fields.state.add_balance(&fields.header.author, &(reward + (reward >> 5) * U256::from(fields.uncles.len())));

		// Bestow uncle rewards
		let current_number = fields.header.number();
		for u in fields.uncles.iter() {
			fields.state.add_balance(u.author(), &(reward * U256::from(8 + u.number() - current_number) >> 3));
		}
